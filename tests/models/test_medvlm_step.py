...
ds_test = CTRATE_Dataset3D(split='train', tokenizer=tokenizer)
...
for idx, batch in tqdm(enumerate(iter(dl))):
    ...
    with torch.no_grad():
        loss_before = model._step(batch, batch_idx=idx, state="train", step=idx*dm.batch_size)
    ...
    # --- LOSS AFTER UPDATE ---
    with torch.no_grad():
        loss_after = model._step(batch, batch_idx=idx, state="train", step=idx * dm.batch_size)
